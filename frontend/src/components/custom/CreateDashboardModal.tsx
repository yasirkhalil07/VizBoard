...
import { createDashboardThunk } from "@/store/thunks/dashboardThunks";
...
export function CreateDashboardModal({ children }: CreateDashboardModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast.success("Dashboard created successfully!");
      setOpen(false);
      setName("");
    } catch (error: any) {
      showToast.error(error.message || "Failed to create dashboard");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-600/20 dark:to-indigo-600/20 p-6 border-b border-slate-100 dark:border-slate-800">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-500 dark:text-slate-400">
              Create a workspace to visualize your data insights.
            </DialogDescription>
          </DialogHeader>
        </div>
